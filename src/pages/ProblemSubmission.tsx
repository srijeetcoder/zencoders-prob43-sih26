...
function ReportProblemPage() {
  ...
  // Ask Gemini AI to review description and formulate accurate title / heading
  const handleGeminiAiReview = async () => {
    if (!description.trim()) {
      setMediaError("Please write a brief description first for Gemini AI to review.");
      return;
    }
        ...
        <div className="border-b border-slate-100 px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Attach up to 3 photos and 1 video (&le; 1 min). Powered by Gemini AI for accurate headings & multi-stakeholder routing.
            </p>
          </div>

          <button
            type="button"
            onClick={handleGeminiAiReview}
          ...
          >
            {isAiReviewing ? (
              <>
                ...
                <span>Gemini AI Reviewing...</span>
              </>
            ) : (
              <>
                <Sparkles className="h-3.5 w-3.5 text-emerald-600" />
                <span>Ask Gemini AI to Polish & Review</span>
              </>
            )}
          </button>
        </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50/90 to-teal-50/70 p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-1.5 font-mono text-[11px] font-bold text-emerald-800 uppercase tracking-wider">
                  <Sparkles className="h-3.5 w-3.5 text-emerald-600" />
                  Gemini AI Problem Analysis & Heading
                </span>
          ...
          <div className="flex flex-1 flex-col gap-4">
            <div className="flex flex-col">
              <label className="mb-1.5 block text-xs font-bold text-slate-800">
                Problem Title / Heading (Auto-generated by Gemini AI or custom)
              </label>
              <input
                type="text"
                value={title}
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>Indexing with Gemini AI...</span>
              </>
            ) : (
              <>
