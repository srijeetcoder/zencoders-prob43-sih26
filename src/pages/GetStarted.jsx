...
function makeId() {
  return `JS-2026-${String(1000 + Math.floor(Math.random() * 9000))}`
}
...
export default function GetStarted() {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    const payload = {
      rawDescription: `${title}. ${tried}`,
      text: title,
      district: district.trim() || 'Ranchi',
      category,
      citizenName: activeActor.label,
    }

    try {
      const response = await citizenApi.submitGrievance(payload)
      const newRecord = {
        id: response.ticketId || makeId(),
        problemId: response.problemId,
        actor: activeActor,
        title: title.trim() || 'Community Grievance',
        category: response.classifiedDomain || category,
        place: `${district.trim() || 'Ranchi'}, ${state.trim() || 'Jharkhand'}`,
        district: district.trim() || 'Ranchi',
        state: state.trim() || 'Jharkhand',
        urgency,
        tried: tried.trim() || 'Local infrastructure bottleneck requiring lab telemetry matching.',
        beneficiaries: beneficiaries.trim() || '18,000 residents across community clusters',
        skillsNeeded: (response.requiredDisciplines && response.requiredDisciplines.join(', ')) || skillsNeeded.trim() || 'IoT Telemetry & Embedded Systems',
        detectedDialect: response.detectedDialect || 'Nagpuri / Hinglish',
        assignedDepartment: response.assignedDepartment || 'Govt of Jharkhand Nodal Task Force',
        date: 'Just now',
      }
      ...
      const fallbackRecord = {
        ...
        state: state.trim() || 'Jharkhand',
        urgency,
        tried: tried.trim() || 'Traditional borewells dry out during summer.',
        beneficiaries: beneficiaries.trim() || '18,000 residents across 14 hamlets',
        skillsNeeded: skillsNeeded.trim() || 'IoT Telemetry / Water Quality Engineering',
        detectedDialect: 'Nagpuri / Hindi',
        assignedDepartment: 'Dept of Drinking Water & Sanitation, Jharkhand',
        date: 'Just now',
      }
  ...
  return (
    <div id="get-started" className="min-h-screen flex flex-col justify-between bg-paper font-sans text-ink">
      <Nav />
              ...
              <div className="mt-6 rounded-2xl border border-line bg-paper p-6 shadow-sm">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 text-xs">
                    ...
                    <div className="rounded-xl border border-line bg-paper-2 p-3">
                      <p className="font-bold text-ink-3 uppercase text-[10px]">Assigned Department</p>
                      <p className="font-semibold text-ink mt-0.5">{submittedData.assignedDepartment}</p>
                    </div>
                  </div>
                </div>
              </div>

              {/* Citizen Post-Submission Authentication & OTP Claim Card */}
              <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5 shadow-xs">
                <div className="flex items-center gap-2 text-emerald-800 font-bold text-sm">
                  <ShieldCheck className="h-4 w-4 text-emerald-600" />
                  <span>Claim Ticket & Enable Real-Time SMS/WhatsApp Telemetry</span>
                </div>
                <p className="text-xs text-emerald-700 mt-1">
                  Authenticate your mobile number to bind ownership of ticket <strong>{submittedData.id}</strong> and receive automated lab milestones.
                </p>
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => alert(`Ticket ${submittedData.id} linked successfully to citizen identity! You can now track live lab progress.`)}
                    className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 transition-all shrink-0"
                  >
                    Verify & Claim
                  </button>
                </div>
