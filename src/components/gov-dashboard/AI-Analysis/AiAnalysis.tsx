...
const DOMAIN_CHART_DATA = [
  { name: "Infrastructure", count: 348, color: "#153157" },
  { name: "Public Health", count: 212, color: "#0d9488" },
  { name: "Energy & Grid", count: 285, color: "#f59e0b" },
  { name: "Environment", count: 172, color: "#10b981" },
  { name: "Education", count: 151, color: "#6366f1" },
  { name: "Agriculture", count: 118, color: "#84cc16" },
];

const SEVERITY_CHART_DATA = [
  { name: "Critical Risk (Priority 1)", value: 38, count: 472, color: "#ef4444" },
  { name: "High Risk (Priority 2)", value: 34, count: 421, color: "#f59e0b" },
  { name: "Moderate Risk (Priority 3)", value: 28, count: 347, color: "#10b981" },
];
...
function AiAnalysis() {
  ...
  const executeAnalysis = async (clusterToAnalyze?: any) => {
    ...
    if (!target && !customPrompt) return;

    setIsLoadingAnalysis(true);
    try {
      const res = await governmentApi.runCabinetAiAnalysis({
        title: target?.clusterTitle || customPrompt.slice(0, 50),
        ...
        description: customPrompt || target?.representativeProblemSummary || target?.underlyingRootCauseHypothesis,
  ...
  useEffect(() => {
    if (clusters.length > 0 && !analysisResult) {
      executeAnalysis(clusters[0]);
    }
  }, [clusters]);

  const sCurveData = useMemo(() => {
    if (analysisResult?.sCurveTrajectory) {
      return analysisResult.sCurveTrajectory;
    }
    return [
      { month: "M+1", adoptionRatePercentage: 12, hazardIndexReductionPercentage: 18, projectedBeneficiaries: 3200, dmfFundMobilizedLakhs: 3.5 },
      { month: "M+3", adoptionRatePercentage: 38, hazardIndexReductionPercentage: 45, projectedBeneficiaries: 14500, dmfFundMobilizedLakhs: 8.0 },
      { month: "M+6", adoptionRatePercentage: 78, hazardIndexReductionPercentage: 74, projectedBeneficiaries: 41000, dmfFundMobilizedLakhs: 14.5 },
      { month: "M+9", adoptionRatePercentage: 90, hazardIndexReductionPercentage: 86, projectedBeneficiaries: 58000, dmfFundMobilizedLakhs: 17.0 },
      { month: "M+12", adoptionRatePercentage: 96, hazardIndexReductionPercentage: 93, projectedBeneficiaries: 72000, dmfFundMobilizedLakhs: 19.5 },
    ];
  }, [analysisResult]);
    ...
    <div className="px-6 py-6 sm:px-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Vector Clustering (pgvector text-embedding-004) &bull; Strict Domain RAG &bull; Hardware BoMs &bull; S-Curve Trajectories &bull; DMF Allocation
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="self-end sm:self-auto pt-4 sm:pt-0">
              <span className="text-[11px] text-slate-500 block">
                Integrated with Gemini 1.5 Flash / 2.0 Flash with automatic RAG grounding
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            <p className="text-xl font-bold text-navy-900">1,240</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">High/Critical Hazard Clusters</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            <p className="text-xl font-bold text-teal-700">100%</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            <p className="text-xl font-bold text-amber-700">
              {analysisResult ? `${(analysisResult.confidence * 100).toFixed(1)}%` : "94.6%"}
            </p>
      ...
      {/* Real-time Interactive Charts (Cases by Domain & Severity Donut) */}
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Live distribution across Jharkhand civic sectors</p>
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={DOMAIN_CHART_DATA} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                ...
                <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                  {DOMAIN_CHART_DATA.map((entry, index) => (
        ...
        {/* Severity Donut Chart - Completely Centered & Fixed */}
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              District: <strong className="text-navy-900">{analysisResult?.district || selectedCluster?.district || "Dumka"}</strong> &bull; Domain: <strong className="text-navy-900">{analysisResult?.domain || selectedCluster?.domain || "Energy"}</strong>
            </p>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4">
              ...
              <p className="mt-2 text-sm text-slate-700 leading-relaxed font-medium">
                {analysisResult?.executiveSummary || "Systemic intervention blueprint synthesized with RAG-grounded engineering evidence."}
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-amber-50/60 border border-amber-200 p-4">
              ...
              <p className="mt-2 text-sm text-slate-800 leading-relaxed">
                {analysisResult?.systemicRootCauseSynthesis || selectedCluster?.underlyingRootCauseHypothesis || "Lack of automated telemetry and deferred preventative maintenance."}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-2xl bg-teal-50/70 border border-teal-200 p-4">
                ...
                <p className="mt-1 text-2xl font-bold text-teal-900">
                  ₹ {analysisResult?.dmfAllocationStrategy?.dmfGrantAmountLakhs || 14.8} Lakhs
                </p>
                <p className="text-[11px] text-teal-700 mt-1">
                  District Mineral Foundation Trust (Trust Rules 2016)
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-emerald-100 text-emerald-800 px-3 py-1 text-xs font-bold">
                  BoM Compliance: {analysisResult?.bomComplianceScore || 100}%
                </span>
                <span className="rounded-full bg-navy-900 text-white px-3 py-1 text-xs font-bold">
                  Total: ₹ {(analysisResult?.bomTotalCostINR || 453500).toLocaleString("en-IN")}
                </span>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {(analysisResult?.hardwareBoM || []).map((item: any, idx: number) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 text-right font-mono font-bold text-teal-700">₹ {(item.totalCostINR || 50000).toLocaleString("en-IN")}</td>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {sCurveData.slice(0, 4).map((m: any) => (
                <div key={m.month} className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-center">
                  ...
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider">Beneficiaries Covered</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-slate-300 bg-slate-50/80 p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div>
                  <span className="text-[11px] font-mono font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded">
                    ADMINISTRATIVE ORDER #{analysisResult?.districtActionDirective?.orderReference || "GOV-JH-WAR-2026-8812"}
                  </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <span className="text-sm font-bold text-navy-900">
                    {analysisResult?.districtActionDirective?.designatedNodalOfficer || `Deputy Commissioner, ${selectedDistrict}`}
                  </span>
                </div>
                <div>
                  <strong className="text-slate-500 block">Mandated SLA Window:</strong>
                  <span className="text-sm font-bold text-rose-600">
                    {analysisResult?.districtActionDirective?.mandatedSlaDays || 14} Calendar Days
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-1.5 text-xs text-slate-800">
                  {(analysisResult?.districtActionDirective?.immediateDirectives || [
                    "Immediate joint physical audit of hotspot nodes within 48 hours",
                    "Mobilize emergency fast-track sanction under District Mineral Fund (DMF)",
                    "Establish continuous live edge telemetry feed with State War Room console"
              ...
              <div className="rounded-xl bg-rose-50/70 border border-rose-200 p-3 text-xs text-rose-900">
                <strong>Statutory Compliance Warning:</strong> {analysisResult?.districtActionDirective?.penalConsequencesOfDefault || "Section 12 of Jharkhand State Right to Public Services Act."}
              </div>
            </div>
          </div>
        )}
